...
import { CheckCircle2, AlertCircle, Loader2, X, FileText, Upload, ChevronDown, ChevronUp } from "lucide-react";
import { useUpload, type UploadSession } from "@/providers/upload-provider";
import { cn } from "@/lib/utils";

function UploadCard({ session, onDismiss }: { session: UploadSession; onDismiss: () => void }) {
    const [collapsed, setCollapsed] = useState(false);

    const { files, overallStatus, errorMessage } = session;
    ...
    return (
        <div
            className={cn(
                "w-[340px] rounded-2xl shadow-2xl overflow-hidden border bg-white dark:bg-slate-900",
                isDone
            ...
            >
                <div className="flex items-center gap-2.5 min-w-0">
                    {isUploading || isCreating ? (
                        <div className="flex items-center justify-center w-7 h-7 rounded-full bg-blue-100 dark:bg-blue-900/50 shrink-0">
                            <Loader2 size={14} className="animate-spin text-blue-600 dark:text-blue-400" />
                        </div>
                    ) : isDone ? (
                        <div className="flex items-center justify-center w-7 h-7 rounded-full bg-emerald-100 dark:bg-emerald-900/50 shrink-0">
                            <CheckCircle2 size={14} className="text-emerald-600 dark:text-emerald-400" />
                        </div>
                    ) : (
                        <div className="flex items-center justify-center w-7 h-7 rounded-full bg-red-100 dark:bg-red-900/50 shrink-0">
                    ...
                    {files.map((f, i) => (
                        <div key={i} className="space-y-1">
                            <div className="flex items-center gap-2 min-w-0">
                                ...
                                {f.status === "done" && <CheckCircle2 size={13} className="text-emerald-500 shrink-0" />}
...
export function UploadToast() {
    const { sessions, dismiss } = useUpload();
    if (!sessions.length) return null;

    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 items-end">
            {sessions.map(session => (
                <UploadCard key={session.id} session={session} onDismiss={() => dismiss(session.id)} />
            ))}
        </div>
    );
}
